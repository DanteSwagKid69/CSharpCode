...
namespace HangMan
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string[] randomWordsArray = {"axel", "people",
            ...
            while (gameIsAlive)
            {
                ...
                while (!gameIsWon(hiddenLetterArray))
                {
                    Console.Clear();
                    //skriv ut list
                    Console.Write("Gissade bokstäver: ");
                    Console.ForegroundColor = ConsoleColor.Red;
                    printList(guessedLettersList);
                    Console.ResetColor();
                    Console.WriteLine(" |" + wrongGuessesLeft + " fel gissningar kvar");
                    Console.Write("\n");
                    drawHangMan(wrongGuessesLeft);
                    //skriv ut array
                    printArray(hiddenLetterArray);
                    Console.WriteLine("\nSkriv in en bokstav du vill testa:");
                    string inputString = Console.ReadLine();
                    while (!GoodInput(inputString))
                    {
                        Console.WriteLine("Du skrev inte in en giltig bokstav, försök igen: ");
                        inputString = Console.ReadLine();
                    }

                    char input = char.Parse(inputString);
                    while (hiddenLetterArray.Contains(input) || guessedLettersList.Contains(input))
                    {
                        Console.WriteLine("Du har redan skrivigt " + input + ", skriv någon annan");
                    ...
                    for (int i = 0; i < word.Length; i++)
                    {
                        ...
                        {
                            hiddenLetterArray[i] = input;
                        }
                    }

                    if (!hiddenLetterArray.Contains(input))
                    {
                        guessedLettersList.Add(input);
                        wrongGuessesLeft--;
                    }
                    if (gameIsWon(hiddenLetterArray))
                    {
                        Console.Clear();
                    ...
                    if (wrongGuessesLeft < 1)
                    {
                        Console.Clear();
                        Console.ForegroundColor = ConsoleColor.Red;
                        Console.WriteLine("Du gissade inte ordet");
                        Console.ForegroundColor = ConsoleColor.Yellow;
                        Console.WriteLine("Ordet var " + word);
                        break;
                    }
                ...
                drawHangMan(wrongGuessesLeft);
                Console.WriteLine("Vill du spela eng gång till? [ja/nej]");
                ...
                while (true)
                {
                    ...
                    {
                        Console.Write("Du skrev inte in någon av alternativen, försök igen: ");
        ...
        static bool gameIsWon(char[] charArray)
        {
        ...
        static bool GoodInput(string input)
        {
            if (input.Length == 1)
            {
        ...
        static void drawHangMan(int numberOfGuess)
        {
